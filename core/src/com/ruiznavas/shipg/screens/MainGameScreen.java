...
public class MainGameScreen implements Screen {
	...
	public static final float TIEMPO_MINIMO_SPAWN_ASTEROIDE = 0.1f;
	public static final float TIEMPO_MAXIMO_SPAWN_ASTEROIDE = 0.3f;
	...
	public MainGameScreen(ShipGame game) {
		...
		random = new Random();
		timerSpawnAsteroide = random.nextFloat() * (TIEMPO_MAXIMO_SPAWN_ASTEROIDE - TIEMPO_MINIMO_SPAWN_ASTEROIDE)
				+ TIEMPO_MINIMO_SPAWN_ASTEROIDE;

		roll = 2;
		rollTimer = 0;
		shootTimer = 0;
		rolls = new Animation[5];
		TextureRegion[][] rollSpriteSheet = TextureRegion.split(new Texture("ship.png"), ANCHO_PIXEL_SHIP,
				ALTO_PIXEL_SHIP);
	...
	public void render(float delta) {
		...
		if ((esDerecha() || esIzquierda()) && shootTimer >= TIEMPO_ESPERA_DISPARO) {
			shootTimer = 0;
			// Dejamos de mostrar las instrucciones
		...
		// Dibujamos las instrucciones de los controles
		if (mostrarControles) {
			if (ShipGame.ES_MOVIL) {
				// Dibujamos el control izquierda
				game.getBatch().setColor(Color.RED);
				game.getBatch().draw(controles, 0, 0, ShipGame.ANCHO / 2, ShipGame.ALTO, 0, 0, ShipGame.ANCHO / 2,
						ShipGame.ALTO, false, false);

				game.getBatch().setColor(Color.BLUE);
				game.getBatch().draw(controles, ShipGame.ANCHO / 2, 0, ShipGame.ANCHO / 2, ShipGame.ALTO, 0, 0,
						ShipGame.ANCHO / 2, ShipGame.ALTO, false, false);

				game.getBatch().setColor(Color.WHITE);
			}else {
				GlyphLayout capaInstrucciones = new GlyphLayout(fuentePuntuacion, "Press Left/Right to Shoot",
						Color.WHITE, ShipGame.ANCHO -50, Align.left, true);
				
				fuentePuntuacion.draw(game.getBatch(), capaInstrucciones, ShipGame.ANCHO/2 - capaInstrucciones.width/2, 100);
			}
		}
	...
	private boolean esDerecha() {
		return Gdx.input.isKeyPressed(Keys.RIGHT)
				|| (Gdx.input.isTouched() && game.camaraJuego.getInputEnMundoJuego().x >= ShipGame.ANCHO / 2);
	}
	...
	private boolean esSoloDerecha() {
		return Gdx.input.isKeyJustPressed(Keys.RIGHT)
				|| (Gdx.input.justTouched() && game.camaraJuego.getInputEnMundoJuego().x >= ShipGame.ANCHO / 2);
	}
